...
<head>
    ...
    <link rel="stylesheet" href="public/assets/index.css?v=<?php echo time(); ?>" />
        ...
        <?php if (!isset($_SESSION['login'])) { ?>
                ...
                <a href="?client=categoryShow">
                    <div class="category">
                        ...
                        <ul class="sub-menu">
                            ...
                            ?>
                                <li><a href="?client=categoryShow&iddm=<?= $dm_id ?>"><?= $dm_name ?> </a></li>
            ...
            <div class="search-index">
                <form class="form-index" action="">
                    <input type="hidden" name="client" value="search" id="">
                    <input type="" name="search" id="search" placeholder="Tìm kiếm" />
                    <button type="submit" class="btn-form-index"><i id="icon-search" class="bx bx-search"></i></button>
            ...
            foreach ($list_account as $account): extract($account);
                if ($tk_user == $_SESSION['login']) { ?>
                    <div class="logo">
                        <h1 class="logo-style"><a href="?act=client&iduser=<?= $tk_id ?>">FourSmart</a></h1>
                    </div>
                    <div class="category-full">
                        <div class="category">
                            <!-- <i id="icon-login" class="bx bx-menu"></i> -->
                            ...
                            <ul class="sub-menu">
                                <?php foreach ($list_category as $category) : extract($category)
                                ?>
                                    <li><a href="?client=categoryShow&iddm=<?= $dm_id ?>&iduser=<?= $tk_id ?>"><?= $dm_name ?> </a></li>
                                <?php endforeach ?>
                            </ul>
                    ...
                    <div class="search-index">
                        <form class="form-index" action="">
                            <input type="hidden" name="iduser" value="<?= $tk_id ?>" id="">
                            <input type="hidden" name="client" value="search" id="">
                            <input type="" name="search" id="search" placeholder="Tìm kiếm" />
                            <button type="submit" class="btn-form-index"><i id="icon-search" class="bx bx-search"></i></button>
                        </form>
                    ...
                    <div class="cart-full">
                        <a href="?client=cart&iduser=<?= $tk_id ?>">
                            <div class="cart">
                                <i id="icon-cart" class="bx bx-cart"></i>
                                <a class="cart-link" href="?client=cart&iduser=<?= $tk_id ?>">Giỏ hàng</a>
                            </div>
                        </a>
                    </div>
            <?php }
            ...
            <?php } else { ?>
                ...
                <?php } else { ?>
                    <?php foreach ($list_account as $account): extract($account);
                        if ($tk_id == $_GET['iduser']) { ?>
                            <a href="?client=detailProfile&iduser=<?= $tk_id ?>">
                                <div class="login">
                                    <i id="icon-login" class="bx bx-user-circle"></i>
                                    <a class="login-link" href="?client=detailProfile&iduser=<?= $tk_id ?>"><?= $_SESSION['login'] ?></a>
                                </div>
                            </a>
                    <?php  }
                    endforeach ?>
